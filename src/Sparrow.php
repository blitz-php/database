...
class Sparrow
{
    ...
    public function parseConnection($connection)
    {
        ...
        $cfg['type']     = $url['scheme'] ?? $url['path'];
        $cfg['hostname'] = $url['host'] ?? null;
        $cfg['database'] = isset($url['path']) ? substr($url['path'], 1) : null;
        $cfg['username'] = $url['user'] ?? null;
        $cfg['password'] = $url['pass'] ?? null;
        $cfg['port']     = $url['port'] ?? null;
     ...
     * @param array|string $value  Condition value
    ...
    protected function parseCondition($field, $value = null, $join = '', $escape = true)
    {
        if (is_string($field)) {
            ...
            if (is_array($value)) {
                ...
                $value = ($escape && ! is_numeric($value)) ? $this->quote($value) : (string) $value;
            }
     ...
     *
     * @param array|string $field A field name or an array of fields and values.
     ...
     *
     * @param array|string $field Field name or array of field names
     *
     * @return object Self reference
     */
     ...
     *
     * @param array|string $data Array of keys and values, or string literal
     *
     * @return object Self reference
     */
     ...
     *
     * @param array|string SQL statement
     * @param mixed|null $sql
     *
     ...
     *
     * @param array|object|string $db Database connection string, array or object
     *
     ...
     *
     * @param object|string $cache Cache connection string or object
     *
     ...
     *
     * @param array|int|string Search value
     * @param string     $key   Cache key
